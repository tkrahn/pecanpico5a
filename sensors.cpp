...
int getUBatt()
{
  ...
  int adc = analogRead(A0);  // Real read
...
int getUSolar()
{
  ...
  int adc = analogRead(A6);  // Real read
  // mV = ( Uref * adc / 1024bit resolution ) / Voltage divider ratio
  // mV = ( Uref * adc / 1024bit resolution ) / (100 kOhm / (33 kOhm + 100 kOhm)) for PecanPico 5a Solar

  // 
  // Uref = 3.3 V
  long mV = adc * 4389L / 1024L;
