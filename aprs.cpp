...
// Temperature (+100 deg C for all positive values); Write to T# pos 2
    snprintf(temp, 4, "%03d", (int)(bmp085temp + 100));  
...
// adc6 = Usolar; Write to T# pos 4
    analogRead(A6);                      // Disregard the 1st conversion after changing ref
    value = analogRead(A6);          // read ADC
    snprintf(temp, 4, "%03d", value/4L);  // Arduino has 1023 max.; APRS has 255 max.; So divide by 4
    ax25_send_string(temp);               // write 8 bit ADC value
    ax25_send_byte(',');  
...
// Time to lock; Write to T# pos 5
  snprintf(temp, 4, "%03d", time2lock); // TTL in cycle periods
  ax25_send_string(temp);               // write 8 bit value
  ax25_send_byte(',');  
...
// adc3 = Attach a LM50 temp sensor here ; Write to T# pos 5
  ...
  // fill the fourth telemetry value with the latitude
  snprintf(temp, 4, "%03d", int(abs(gps_lat))); 
  ax25_send_string(temp);               // write 8 bit value
  ax25_send_byte(',');   
  
  // fill the fifth telemetry value with the longitude
  snprintf(temp, 4, "%03d", int(abs(gps_lon))); 
  ax25_send_string(temp);               // write 8 bit value
  ax25_send_byte(',');   
*/
...
//    if (bmp085pressure < 1) { bmp085pressure = 1; } //make sure we can calculate the logarithm without errors
//    altitude = ((float)-22069 * log( (double)bmp085pressure ) + (float)255946) * 0.3048 ;

    altitude = 0; // pressure altitude disabled because PP6 has no pressure sensor
...
//    ax25_send_string(itoa(sensors_lm50(), temp, 10));
  ax25_send_string(dtostrf((getUSolar()/1000.0),4,1,temp));
  ax25_send_string("Vs ");
  ...
  if(newPositionStillUnknown)
  {
    ...
    {
      loss_of_gps_counter = 15; // will stay at 15 at permanent gps loss.
    }
