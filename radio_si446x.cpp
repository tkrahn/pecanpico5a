/* pecanpico6 Si446x Driver copyright (C) 2014  KT5TK
 *
...
//  const char gpio_pin_cfg_command[] = {0x13, 0x04, 0x04, 0x04, 0x02, 0x08, 0x0B, 0x00}; //  Set GPIOs 0..4 to input, GPIO3 LOW; Link NIRQ to CTS; Link SDO to MISO; Max drive strength
  const char gpio_pin_cfg_command[] = {0x13, 0x02, 0x02, 0x04, 0x02, 0x08, 0x11, 0x00}; //  Set GPIOs 0..4 to input, GPIO3 LOW; Link NIRQ to CTS; Link SDO to MISO; Max drive strength
...
void RadioSi446x::setPaPower(unsigned short power)
{
  // Make sure 0 <= power < 128 
  if (power < 0)
  {
    power = 0;
  }
...
//  Serial.println("Setting PA Power");  
  char set_pa_power_command[] = {0x11, 0x22, 0x01, 0x01, power};
  SendCmdReceiveAnswer(5, 1, set_pa_power_command);
...
void RadioSi446x::ptt_on()
{
  ...
  digitalWrite(VCXO_ENABLE_PIN, HIGH);
  reset();
  // turn on the blue LED (GPIO3) to indicate TX
  char gpio_pin_cfg_command2[] = {0x13, 0x02, 0x02, 0x04, 0x03, 0x08, 0x11, 0x00}; //  Set GPIO3 HIGH; Link NIRQ to CTS; Link SDO to MISO; Max drive strength
  SendCmdReceiveAnswer(8, 1, gpio_pin_cfg_command2);
  
  si446x_powerlevel = 60; // 0 ... 127
  setPaPower(si446x_powerlevel);
  start_tx();
...
void RadioSi446x::ptt_off()
{
  ...
  char gpio_pin_cfg_command0[] = {0x13, 0x02, 0x02, 0x04, 0x02, 0x08, 0x11, 0x00}; //  Set GPIOs 0..4 to input, GPIO3 LOW; Link NIRQ to CTS; Link SDO to MISO; Max drive strength
