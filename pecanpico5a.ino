...
//  if (millis() >= next_tx_millis)
  {
    ...
    if (!newPositionStillUnknown || (wd_counter >= 15 + (int)(APRS_PERIOD_SECONDS / 8))) // We have our GPS position or 2 minutes have passed without lease (giving up)
    {
      gpsSetTime2lock( wd_counter - (int)(APRS_PERIOD_SECONDS / 8) );
    ...
    if (Serial.available()) 
      {
        ...
        if (gps_decode(c)) 
        {
          ...
          if(gps_check_satellite())
          {
            ...
            // upset the watchdog so that we double the transmitting rate while ISS is in view
            //wd_counter = (int)((APRS_PERIOD_SECONDS / 8) / 2);
          }
